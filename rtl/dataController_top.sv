...
`endif
	...
	// VIA2 Port A: {glue_ram_size[1:0], nubus_irq_state[5:0]}
	// PA[7:6] are the Mac II GLUE RAM placement latch.  MAME models reads
	// from this port as returning the latch value, and the ROM uses writes to
	// move/probe bank B during RAM sizing.
	// PA[5:0] are NuBus slot IRQ state (active-low, directly from video card slot E = bit 5)
	assign glueRAMSize = via2_pa_o[7:6];
	assign via2_pa_i = {via2_pa_o[7],
	                    via2_pa_o[6],
