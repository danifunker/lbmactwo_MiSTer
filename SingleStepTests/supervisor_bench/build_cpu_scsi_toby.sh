#!/bin/bash
# build_cpu_scsi_toby.sh — same as build_cpu_scsi.sh but for the Toby
...
OUT="${2:-/tmp/cpu_scsi_toby.hda}"

BOOT=build/boot_stub_scsi_toby.bin
PAYLOAD=build/payload_cpu_scsi_toby.bin
RESULTS_SIZE=409600

[[ -x "$RB" ]]    || { echo "rusty-backup-cli not found"; exit 1; }
[[ -f "$BOOT" ]]  || { echo "missing $BOOT — run 'make cpu_scsi_toby'"; exit 1; }
[[ -f "$PAYLOAD" ]] || { echo "missing $PAYLOAD — run 'make cpu_scsi_toby'"; exit 1; }

cp "$TEMPLATE" "$OUT"
PART=/tmp/cpu_scsi_toby_part.dsk
...
RP=/tmp/cpu_scsi_toby_probe.bin
...
echo "/Payload offset:        0x$PAYLOAD_OFFSET  (boot_stub_scsi_toby.s expects 0x51600)"
