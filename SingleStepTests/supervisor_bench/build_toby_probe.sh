#!/bin/bash
# build_toby_probe.sh — assemble a SCSI boot image with the Toby probe.
# Boot it on real Mac II with Toby, then describe what you see:
...
OUT="${2:-/tmp/toby_probe.hda}"

BOOT=build/boot_stub_toby_probe.bin

[[ -x "$RB" ]]    || { echo "rusty-backup-cli not found"; exit 1; }
[[ -f "$BOOT" ]]  || { echo "missing $BOOT — run 'make toby_probe'"; exit 1; }

cp "$TEMPLATE" "$OUT"
PART=/tmp/toby_probe_part.dsk
