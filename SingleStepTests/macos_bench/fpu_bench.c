...
int main(void)
{
    ...
    printf("Running %d FPU tests against real 68881...\n", (int)N_TESTS);
    ...
    for (i = 0; i < (int)N_TESTS; i++) {
        ...
        fputc('{', f);
        fprintf(f, "\"name\":"); write_json_name(f, tests[i].name);
    ...
    printf("Done. %d tests written under \"%s\".\n", (int)N_TESTS, FPU_OUTPUT_DIR);
    return 0;
}
