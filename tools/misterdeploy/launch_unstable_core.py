...
def main():
    ...
    # MiSTer persists the main-menu cursor across reboots, so the root menu does
    # NOT reliably reopen on row 0 -- blind "down x N from the top" then lands on
    # the wrong folder (observed: launched Genesis/MiSTerLaggy instead of the
    # _Unstable core). The main menu CLAMPS at the top (no wrap), so a burst of
    # "up" presses homes the cursor deterministically before we count down.
    home = ["up"] * (f_total + 3)
    keys = (home + ["down"] * f_steps + ["confirm", "sleep:1.2"]
